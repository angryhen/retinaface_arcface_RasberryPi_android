...
void Live::LoadModel(std::vector<ModelConfig> &configs, AAssetManager *mgr) {
    configs_ = configs;
    ...
    for (int i = 0; i < model_num_; ++i) {
        ...
        std::string param = configs_[i].name + ".param";
        std::string model = configs_[i].name + ".bin";
        net->load_param(mgr, param.c_str());
        net->load_model(mgr, model.c_str());
